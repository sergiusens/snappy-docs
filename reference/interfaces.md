...
| `autopilot-introspection` | Can be introspected by autopilot _(snapd 2.24+)_. | no | no |  |
...
| `framebuffer` | Can use the universal framebuffer (/dev/fb[0-9]*, _(snapd 2.22+)_). | no | no |  |
...
| `maliit` | Can access snaps providing the maliit input interface (_snapd 2.24+_). | no | no |  |
...
| `network-setup-control` | Can read/write network setup configuration files. This is restricted because it gives access to system network configuration which can container network security details _(snapd 2.22+)_. | no | no |  |
...
| `thumbnailer-service` | Can access snaps providing the thumbnailer service interface (_snapd 2.22+_). | no | no |  |
